{
  ...
  '#':: d.pkg(name='clickHouseInstallation', url='', help='"define a set of Kubernetes resources (StatefulSet, PVC, Service, ConfigMap) which describe behavior one or more ClickHouse clusters"'),
  ...
  spec: {
    ...
    reconciling: {
      ...
      '#withConfigMapPropagationTimeout':: d.fn(help='"timeout in seconds when `clickhouse-operator` will wait when applied `ConfigMap` during reconcile `ClickhouseInstallation` pods will updated from cache\\nsee details: https://kubernetes.io/docs/concepts/configuration/configmap/#mounted-configmaps-are-updated-automatically\\n"', args=[d.arg(name='configMapPropagationTimeout', type=d.T.integer)]),
    ...
    '#templating':: d.obj(help='"optional, define policy for auto applying ClickHouseInstallationTemplate inside ClickHouseInstallation"'),
    templating: {
      '#withChiSelector':: d.fn(help='"optional, define selector applying ClickHouseInstallationTemplate inside ClickHouseInstallation"', args=[d.arg(name='chiSelector', type=d.T.object)]),
      withChiSelector(chiSelector): { spec+: { templating+: { chiSelector: chiSelector } } },
      '#withChiSelectorMixin':: d.fn(help='"optional, define selector applying ClickHouseInstallationTemplate inside ClickHouseInstallation"\n\n**Note:** This function appends passed data to existing values', args=[d.arg(name='chiSelector', type=d.T.object)]),
      withChiSelectorMixin(chiSelector): { spec+: { templating+: { chiSelector+: chiSelector } } },
      '#withPolicy':: d.fn(help='"when defined as `auto` inside ClickhouseInstallationTemplate, it will auto add into all ClickHouseInstallation, manual value is default"', args=[d.arg(name='policy', type=d.T.string)]),
    ...
    '#withNamespaceDomainPattern':: d.fn(help='"custom domain suffix which will add to end of `Service` or `Pod` name, use it when you use custom cluster domain in your Kubernetes cluster"', args=[d.arg(name='namespaceDomainPattern', type=d.T.string)]),
    withNamespaceDomainPattern(namespaceDomainPattern): { spec+: { namespaceDomainPattern: namespaceDomainPattern } },
    '#withRestart':: d.fn(help="\"This is a 'soft restart' button. When set to 'RollingUpdate' operator will restart ClickHouse pods in a graceful way. Remove it after the use in order to avoid unneeded restarts\"", args=[d.arg(name='restart', type=d.T.string)]),
    withRestart(restart): { spec+: { restart: restart } },
    '#withStop':: d.fn(help='"Allow stop all ClickHouse clusters described in current chi.\\nStop mechanism works as follows:\\n - When `stop` is `1` then setup `Replicas: 0` in each related to current `chi` StatefulSet resource, all `Pods` and `Service` resources will desctroy, but PVCs still live\\n - When `stop` is `0` then `Pods` will created again and will attach retained PVCs and `Service` also will created again\\n"', args=[d.arg(name='stop', type=d.T.string)]),
    withStop(stop): { spec+: { stop: stop } },
    '#withTaskID':: d.fn(help='"Allows to define custom taskID for named update operation and watch status of this update execution in .status.taskIDs field.\\nBy default every update of chi manifest will generate random taskID\\n"', args=[d.arg(name='taskID', type=d.T.string)]),
    withTaskID(taskID): { spec+: { taskID: taskID } },
    '#withTroubleshoot':: d.fn(help="\"allows troubleshoot Pods during CrashLoopBack state, when you apply wrong configuration, `clickhouse-server` wouldn't startup\"", args=[d.arg(name='troubleshoot', type=d.T.string)]),
